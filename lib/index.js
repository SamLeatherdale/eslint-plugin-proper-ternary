...
module.exports = {
	configs: {
		"getify-says": {
			plugins: [ "@samldev/proper-ternary", ],
			rules: {
				"@samldev/proper-ternary/nested": [ "error", { "else": true, "depth": 10, }, ],
				"@samldev/proper-ternary/parens": [ "error", { "ternary": false, "call": false, "object": false, }, ],
				"@samldev/proper-ternary/where": [ "error", { "return": false, }, ],
			},
		},
	},
	rules: {
		"nested": {
			meta: {
				...
				docs: {
					...
					url: "https://github.com/samleatherdale/eslint-plugin-proper-ternary/#rule-nested",
				},
				schema: [
					{
						type: "object",
		...
		"parens": {
			meta: {
				...
				docs: {
					...
					category: "Best Practices",
					url: "https://github.com/samleatherdale/eslint-plugin-proper-ternary/#rule-parens",
				},
				schema: [
					{
						type: "object",
				...
				messages: {
					...
					needParensTernary: "Ternary clause expression requires enclosing ( .. ) [ternary]",
					needParensComparison: "Ternary clause expression requires enclosing ( .. ) [comparison]",
					needParensLogical: "Ternary clause expression requires enclosing ( .. ) [logical]",
					needParensCall: "Ternary clause expression requires enclosing ( .. ) [call]",
					needParensObject: "Ternary clause expression requires enclosing ( .. ) [object]",
					needParensSimple: "Ternary clause expression requires enclosing ( .. ) [simple]",
					needParensComplex: "Ternary clause expression requires enclosing ( .. ) [complex]",
					needParensUnary: "Ternary clause expression requires enclosing ( .. ) [unary]",
				},
			},
			create(context) {
				var defaultsOnly = context.options.length == 0;
				...
				// Added options
				var unaryMode = defaultsOnly || !("unary" in extraOptions) || extraOptions.unary === true;
				var complexMode = defaultsOnly || !("complex" in extraOptions) || extraOptions.complex === true;
					...
					"ConditionalExpression": function enter(node) {
						for (let clause of [node.test,node.consequent,node.alternate,]) {
							let exprType =
								...
								(clause.type == "UnaryExpression" && clause.operator == "!") ? "unary" :
							...
							const conditions = {
								'ternary': () => ternaryMode && exprType == "ternary",
								'comparison': () => comparisonMode && exprType == "comparison",
								'logical': () => logicalMode && exprType == "logical",
								'call': () => callMode && exprType == "call",
								'object': () => objectMode && exprType == "object",
								'simple': () => simpleMode && exprType == "simple",
								'complex': () => complexMode && exprType == "complex",
								'unary': () => unaryMode && exprType == "unary",
							}
							const hasParens = parensSurrounding(sourceCode,clause);
							Object.entries(conditions).forEach(([key, value]) => {
								if (value() && !hasParens) {
									context.report({
										node: clause,
										messageId: `needParens${key.charAt(0).toUpperCase() + key.slice(1)}`,
									});
								}
							})
						}
		...
		"where": {
			meta: {
				...
				docs: {
					...
					category: "Best Practices",
					url: "https://github.com/samleatherdale/eslint-plugin-proper-ternary/#rule-where",
				},
				schema: [
					{
						type: "object",
